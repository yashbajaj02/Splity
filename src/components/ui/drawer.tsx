...
const DrawerOverlay = React.forwardRef<DrawerOverlayRef, DrawerOverlayProps>(
  ({ className, ...props }, ref) => (
    <DrawerPrimitive.Overlay
      ref={ref}
      className={cn("fixed inset-0 z-50 bg-black/40 backdrop-blur-sm transition-all duration-300", className)}
      {...props}
    />
  ),
);
  ...
  ({ className, children, ...props }, ref) => (
    <DrawerPortal>
      ...
      <DrawerPrimitive.Content
        ref={ref}
        className={cn(
          "fixed inset-x-0 bottom-0 z-50 mt-24 flex h-auto flex-col rounded-t-3xl border bg-background shadow-[var(--shadow-3)]",
          className,
        )}
        {...props}
      >
...
const DrawerDescription = React.forwardRef<DrawerDescriptionRef, DrawerDescriptionProps>(
  ({ className, ...props }, ref) => (
