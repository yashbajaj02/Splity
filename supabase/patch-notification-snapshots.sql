-- Run in Supabase SQL Editor to fix "Someone" + missing QR on Activity

alter table public.notifications add column if not exists sender_username text;
alter table public.notifications add column if not exists sender_upi text;
...
where n.sender_id = p.id
  and (n.sender_username is null or n.sender_upi is null);
